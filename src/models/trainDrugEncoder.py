...
parser.add_argument(
    ...
    type=bool,
    default=True,
    help='is the ground truth similarity label continuous?')
...
print("{0}: Executing script: trainCDR-smcRBM.py".format(get_time()))

# DataPath
#  drug_path = f'{fdir}/../../data/drug_response/drugPairs4fewShot.csv'
drug_path = f'{fdir}/../../data/drug_response/drugPairs4fewShotContLabel.csv'
...
def use_parser(argv):
    ...
    inputs['arch']['continuous'] = args.continuous
...
def run(argv):
    ...
    model = SiameseNeuralNet(inputShape=256, **inputs['arch'])
 
    # Train drug encoder model
    model_path = os.path.join(inputs['out']['paths']['models'], inputs['out']['fname'])
    history = model.fit(drug_path, 
